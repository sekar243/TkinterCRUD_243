...
import tkinter as tk  # library GUI
import tkinter.messagebox as msg  # untuk popup message
from tkinter import ttk # untuk tabel Treeview

# -------------------- DATABASE --------------------
def koneksi():
       # membuka / membuat file database SQLite
...
def create_table():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS nilai_siswa (
            id INTEGER PRIMARY KEY AUTOINCREMENT,     -- nomor otomatis
            nama_siswa TEXT NOT NULL,                 -- nama siswa
            biologi INTEGER,                         -- nilai biologi
            fisika INTEGER,                          -- nilai fisika
            inggris INTEGER,                         -- nilai inggris
            prediksi_fakultas TEXT                      -- hasil prediksi
        )
    """)
    con.commit()
    con.close()
...
def read_siswa():
    con = koneksi()
    cur = con.cursor()
    cur.execute("SELECT * FROM nilai_siswa ORDER BY id")  # mengambil semua data
    rows = cur.fetchall()
    con.close()
    return rows
...
class AppNilai(tk.Tk):
    ...
    def load_data(self):
        for row in self.tree.get_children():
            self.tree.delete(row)       # hapus isi tabel lama

        rows = read_siswa()     # ambil data dari DB
        for r in rows:
            self.tree.insert("", tk.END, values=r)     # tampilkan di tabel
